...
def get_api_key():
    ...
    if not api_key:
        st.error("Chave da API da OpenAI não encontrada. Configure st.secrets['OPENAI_API_KEY'] ou a variável de ambiente OPENAI_API_KEY.")
        st.stop()
    return api_key
...
def load_resources(api_key):
    ...
    try:
        ...
        llm = ChatOpenAI(
            model="gpt-3.5-turbo-16k",  # Modelo com contexto maior
        ...
        # Limitar o número de documentos recuperados para reduzir o tamanho do contexto
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        AIMessage(content="E aí, turma! Sou a LucIA, aluna da pós-graduação em IA em Negócios. Agora tenho acesso aos materiais do curso! Como posso ajudar você hoje? Vamos lá!")
    ]
